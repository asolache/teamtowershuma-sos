...
export default class SopsView {
    ...
    async getHtml() {
        ...
        <style>
            ...
            .sv-modal h3 { margin:0 0 0.8rem 0; color:var(--text-main); }
            .sv-modal label { display:block; color:var(--text-secondary); font-size:0.78rem; margin-top:0.7rem; margin-bottom:0.25rem; }
            .sv-modal input, .sv-modal select, .sv-modal textarea { width:100%; box-sizing:border-box; background:var(--bg-dark); color:var(--text-main); border:1px solid var(--border-default); border-radius:5px; padding:0.5rem; font-size:0.85rem; font-family:inherit; }
            .sv-modal textarea { min-height:80px; resize:vertical; }
            .sv-step  { background:var(--bg-dark); border:1px solid var(--border-default); border-radius:5px; padding:0.6rem; margin-top:0.5rem; }
            ...
            .sv-bulk-row    { display:flex; align-items:center; gap:0.6rem; padding:0.4rem 0.5rem; border-bottom:1px solid var(--border-default); font-size:0.78rem; }
            .sv-bulk-row .role { flex:1; color:var(--text-main); }
            .sv-bulk-row .stat { color:var(--text-muted); font-size:0.72rem; }
            .sv-bulk-icon   { font-size:1rem; width:1.2rem; text-align:center; }
            .sv-progressbar { background:var(--bg-elevated); border-radius:4px; overflow:hidden; height:6px; margin:0.5rem 0; }
        ...
        root.innerHTML = `
            <div class="sv-modal" id="svBulkBg">
                <div class="sv-modal-inner" style="max-width:680px;">
                    ...
                    <p style="color:var(--text-secondary);font-size:0.78rem;">Proyecto: <strong>${this._esc(this.project.nombre || this.project.id)}</strong> · ${roles.length} roles · ${skipping} ya con SOP · <strong>${missing} faltantes</strong></p>

                    <div style="margin-top:0.7rem;">
                        <div class="sv-thinking" id="svBulkStatus" style="display:none;">
                            <span>🧠 La IA está pensando</span>
                            <span class="dots"><span></span><span></span><span></span></span>
                            <span id="svBulkPhase" style="color:var(--text-secondary);font-size:0.78rem;"></span>
                        </div>
                    </div>

                    <div class="sv-progressbar"><span id="svBulkProgress" style="width:0%;"></span></div>
                    <div id="svBulkSummary" style="color:var(--text-secondary);font-size:0.75rem;margin-bottom:0.7rem;">Listo para empezar.</div>

                    <div style="max-height:280px;overflow-y:auto;border:1px solid var(--border-default);border-radius:6px;padding:0.3rem 0.5rem;">
                        ${rolesHtml}
                    </div>

                    <p style="color:var(--text-muted);font-size:0.72rem;margin-top:0.5rem;">Por defecto se saltan los roles que ya tienen SOP. Cada llamada al LLM tarda 8-25s. Total estimado: ~${Math.max(missing * 12, 5)}s.</p>
    ...
    _render() {
        ...
        const head = `
            <div style="margin-bottom:1.2rem;">
                <h2 style="margin:0;color:var(--text-main);font-size:1.2rem;">${this._esc(this.project.nombre || this.project.id)}</h2>
                <p style="color:var(--text-secondary);font-size:0.8rem;margin:0.3rem 0 0 0;">${this.sops.length} SOP${this.sops.length === 1 ? '' : 's'} propios del proyecto · sector base ${this._esc(this.project.based_on_sector || this.project.sector_id || '?')}</p>
                <p style="color:var(--text-muted);font-size:0.78rem;margin-top:0.5rem;">Para generar nuevos SOPs ve al <a href="/map?project=${this.projectId}" data-link class="sv-link">mapa de valor</a>, selecciona un rol y pulsa <em>📋 Generar SOP del rol con IA</em>.</p>
            </div>
        `;
        ...
        root.innerHTML = `
            <div class="sv-modal" id="svDetailBg">
                <div class="sv-modal-inner">
                    ...
                    <p style="color:var(--text-secondary);font-size:0.78rem;margin:0;">rol: ${this._esc(c.role_ref || '?')} · v${this._esc(c.version || '1.0')} · ${(c.steps || []).length} steps</p>
        ...
        root.innerHTML = `
            <div class="sv-modal" id="svRegenBg">
                <div class="sv-modal-inner">
                    ...
                    <p style="color:var(--text-secondary);font-size:0.78rem;">SOP actual: <strong>${this._esc(c.name || node.id)}</strong> · v${this._esc(c.version || '1.0')}</p>
...
- Cambia el tono a más operativo, menos consultivo." style="min-height:160px;"></textarea>
    ...
    async _executeRegenerate(node, feedback) {
        ...
        preview.innerHTML = `<p style="color:var(--text-secondary);margin-top:1rem;">🤖 Regenerando con feedback…</p>`;
        try {
