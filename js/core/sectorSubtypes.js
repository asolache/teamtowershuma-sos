...
// FIX UX-AUDIT-001 sprint H+ (2026-05-10) · keys realineades amb CNAE.
// La taxonomia Matriu original (A=Salut · B=Habitatge…) NO coincidia
// amb les keys CNAE que retorna `KnowledgeLoader.listSectors()`
// (A=Agriculture · B=Mining…) · seleccionar un sector mostrava subtipus
// d'un altre àmbit. Ara cada subtipus viu sota la lletra CNAE correcta.
// Els IDs dels subtipus es mantenen amb el prefix històric (A-residencia
// segueix sent l'identificador estable · només viu sota R perquè és
// "Salud y Servicios Sociales" a CNAE).
...
export const SECTOR_SUBTYPES = Object.freeze({
    ...
    // B · Industrias Extractivas (sense subtipus Matriu)
    B: [],
    ...
    // E · Suministro de Agua, Saneamiento (sense subtipus Matriu directes)
    E: [],
    // F · Construcción
    F: [
        { id: 'F-coop-construcc', label: 'Cooperativa de construcció',    hint: 'Bioconstrucció · paller · obres comunitàries',               iaContextHint: 'Cooperativa de construcció ecològica: 5-20 sòcies, projectes paller/CLT/bioconstrucció, formació en obra, relació amb arquitectes ètics.' },
    ],
    // G · Comercio al por Mayor y Menor (sense subtipus directes · grups consum a I)
    G: [],
    ...
    // I · Hostelería y Turismo / Food Services
    I: [
    ...
    // Q · Educación Superior y Formación / Higher Ed
    Q: [
...
export function getSubtypeById(sectorId, subtypeId) {
    ...
    // Cerca primer al sector indicat
    const subs = getSubtypesForSector(sectorId);
    const direct = subs.find(s => s.id === subtypeId);
    if (direct) return direct;
    // Backwards compat · projectes amb subtypeId d'un sector antic
    // (taxonomia Matriu pre-CNAE) · cerca a tots els sectors.
    for (const k of Object.keys(SECTOR_SUBTYPES)) {
        const found = SECTOR_SUBTYPES[k].find(x => x.id === subtypeId);
