...
| **KM-001 sprint E1** | **Integración pruner en `Orchestrator.callLLM` (opt-in)** — `pruneFromKb({KB, projectId, task, options, formatOptions})` async helper que carga candidatos del KB (proyecto + nodos públicos sop/soc/market_item con scope:public) y aplica `pruneContextNodes` + `formatNodesForPrompt`. `Orchestrator.callLLM` acepta nuevo param opcional `contextPruning: {enabled, projectId, task, options, formatOptions}` · si enabled=true, prefija el formatted al systemPrompt y emite `result.telemetry.contextPruning = {enabled, selectedCount, skippedCount, candidatesCount, estTokensBefore, estTokensAfter, deltaTokens, taskTags}`. Default off · NO toca el flujo actual de los consumidores existentes (sectorCloner / roleSopGenerator / woAssistant). Toggle global `Orchestrator.setContextPruningEnabled / isContextPruningEnabled` persistente en KB (`sos_context_pruning_enabled`). Vista `/settings` añade card "🧠 KM-001 · Context pruning (sprint E)" con checkbox que alterna el toggle. Tests · 8 asserts puros adicionales en suite "KM-001 sprint C · contextPruner" (mock KB con 5 nodos · valida que pruneFromKb filtra por proyecto + incluye públicos + excluye config + formatted con header). Sprint E2 enganchará el flag en sectorCloner/roleSopGenerator/woAssistant (cuando @alvaro lo active). | ✅ verde |
