...
| **MAT-002-I** | **Matriu reframe · personas, no proyectos** — sprints A+B entregat. **Sprint B** · `js/core/matriuMemberService.js` puro · schema unificat `type='matriu_member'` que fusiona `cohort_seat` (perfil skills+guardian+bio) + `user_identity` (DID+ECDSA+wallets). Helpers · `buildMatriuMember` puro amb validació (availability/status enums · skillsDeclared array · bio ≤500 chars) · `validateMatriuMember` · `migrateCohortSeatToMember(seat)` puro · `mergeIdentityIntoMember(member, identity)` puro · `migrateIdentityToMember(identity, {cohortNumber=99})` (network sense plaza fundacional) · `summarizeMember` · `migrateAllToMatriuMembers(KB, {dryRun})` async escaneja seats+identities → match per displayName/handle → fusiona via merge → cohort 0 si seat · cohort 99 si identity standalone. Tests · 44 asserts puros. UI · `MatriuNetworkView` ara prioritza `matriu_member` si existeixen, sino llegeix `cohort_seat` legacy (cast virtual). Banner ambre "⚠ Schema legacy" amb botó "🔄 Migrar a matriu_member" · dryRun preview amb confirm + aplicació real (alert de stats). Banner verd "✓ Schema unificat" quan ja s'ha migrat. Backwards-compat preservat · els nodes legacy continuen al KB. **Sprint A** · `MatriuNetworkView.js` ruta `/matriu/network` · skin Matriu (crema #f1ebde + Instrument Serif) · directori 108 amb cards completes (avatar colorat per guardian · pill PW · bio · skills · disponibilitat · projectes pills) · 4 filtres · 4 stat-cards · secció seients lliures + CTA reservar. CTAs · landing /matriu i strip Dashboard MAT-002-F. Sprint C pendent · cada projecte mostra els seus miembros amb fit · sprint D toggle public/network al landing. | 🟢 A+B verde · C/D 🟡 |
