...
| **MAT-002-I** | **Matriu reframe · personas, no proyectos** — sprint A entregat. `js/views/MatriuNetworkView.js` ruta `/matriu/network` · skin Matriu coherent (crema #f1ebde + Instrument Serif) · directori dels 108 amb cards complets per cada `cohort_seat` (avatar amb iniciales colorat per guardian · displayName italic · handle mono · pill guardian Pantheon Work · bio · skills (top 6 + count) · disponibilitat · projectes assignats (via swarm_assignment + WO assignedToSeatId)). Filtres · search global (nom · handle · skill · bio) · select guardian (els 12 PW) · select disponibilitat (high/normal/low) · select hasProject. Stats · X/108 ocupades · guardians coberts (Y/12) · skills cobertes (Z/90). Secció `mn-empty-section` amb seients lliures visibles + CTA reservar. CTAs · al landing /matriu enllaç "🌐 Veure els membres del nucli" · al strip Dashboard MAT-002-F botó "🌐 Xarxa de membres". Sprint B pendent · unificar `cohort_seat` + `user_identity` en `matriu_member` · sprint C cada projecte mostra els seus miembros amb fit · sprint D toggle public/network al landing. | 🟢 sprint A verde · B/C/D 🟡 |
